 ...
 *   <li>{@code # Noncompliant@+N} - expects an issue N lines below the annotation line
 *       (for issue lines that cannot carry a preceding comment, e.g. blank lines)</li>
 *   <li>{@code # Noncompliant@LN} - expects an issue on absolute line N
 *       (for issue lines that have no preceding line, e.g. a byte-order-mark on line 1)</li>
...
public final class CheckVerifier {
     ...
     *   <li>Optional {@code @LN} absolute line (issue on line N)</li>
     *   <li>Optional {@code @+N} line offset (issue N lines below the annotation)</li>
    ...
    static List<TestIssue> extractExpectedIssues(String rawContent) {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (matcher.find()) {
                ...
                // Group 2: optional "@LN" absolute line
                // Group 3: optional "@+N" line offset
                // Group 4: optional column spec [[...]]
                // Group 5: optional message {{...}}
                boolean fileLevel = matcher.group(1) != null;
                String absoluteLine = matcher.group(2);
                String lineOffset = matcher.group(3);
                int issueLine;
                if (fileLevel) {
                    issueLine = 0;              // file-level issue (no specific line)
                } else if (absoluteLine != null) {
                    issueLine = Integer.parseInt(absoluteLine);
                } else if (lineOffset != null) {
                    // annotation on 1-based line (i+1) → issue N lines below
                    issueLine = i + 1 + Integer.parseInt(lineOffset);
                } else {
                    issueLine = i + 2;          // annotation on line i → issue on next line
                }

                String columnSpec = matcher.group(4);
