...
class UniqueScenarioNameCheckTest {

    @Test
    void shouldNotRaiseIssueForUniqueNames() {
        CheckVerifier.verifyNoIssues(
                new UniqueScenarioNameCheck(),
                "checks/unique-scenario-name/compliant.feature");
    }

    @Test
    void shouldRaiseIssueForDuplicateScenarioNames() throws IOException {
        UniqueScenarioNameCheck check = new UniqueScenarioNameCheck();
        FeatureParser parser = new FeatureParser();

        String content1 = readFixture("/checks/unique-scenario-name/compliant.feature");
        ...
        // Second file with same scenario names
        String content2 = readFixture("/checks/unique-scenario-name/compliant.feature");
    ...
    @Test
    void shouldRaiseIssueForDuplicateNamesAcrossRuleBlocksInOneFeature() throws IOException {
        UniqueScenarioNameCheck check = new UniqueScenarioNameCheck();
        FeatureParser parser = new FeatureParser();

        String content = readFixture("/checks/unique-scenario-name/rule-blocks-noncompliant.feature");
        FeatureFile file = parser.parse("rule-blocks.feature",
                new ByteArrayInputStream(content.getBytes(StandardCharsets.UTF_8)));
        check.setContext(new FeatureContext(file, null, content));
        FeatureWalker.walk(file, check);

        List<CrossFileIssue> crossFileIssues = check.afterAllFiles();

        assertThat(crossFileIssues).hasSize(1);
        assertThat(crossFileIssues.get(0).uri()).isEqualTo("rule-blocks.feature");
        assertThat(crossFileIssues.get(0).line()).isEqualTo(12);
        assertThat(crossFileIssues.get(0).message()).contains("Access the account");
    }
