...
class ParityValidationTest {
    ...
    @Test
    void everyRuleHasDocumentationPage() {
        for (Class<?> check : CheckList.getAllChecks()) {
            String key = check.getAnnotation(Rule.class).key();
            Path doc = Path.of("..", "docs", "rules", key + ".md");
            assertThat(doc.toFile().isFile())
                    .as("Missing documentation page (docs/rules/%s.md) for rule: %s", key, key)
                    .isTrue();
        }
    }
