...
class FeatureFileMaxLinesCheckTest {
    ...
    @Test
    void shouldCountCarriageReturnOnlyLineBreaks() throws IOException {
        FeatureFileMaxLinesCheck check = new FeatureFileMaxLinesCheck();
        check.setMaxLines(2);

        String content = "Feature: CR line endings\rline two\rline three\rline four";

        FeatureParser parser = new FeatureParser();
        FeatureFile featureFile = parser.parse("cr-only.feature",
                new ByteArrayInputStream(content.getBytes(StandardCharsets.UTF_8)));
        ...
        assertThat(context.getIssues()).hasSize(1);
        assertThat(context.getIssues().get(0).message())
                .contains("This file has 4 lines");
    }

    @Test
    void shouldCountCrLfLineBreaksAsOne() throws IOException {
        FeatureFileMaxLinesCheck check = new FeatureFileMaxLinesCheck();
        check.setMaxLines(2);

        String content = "Feature: CRLF line endings\r\nline two\r\nline three\r\n";

        FeatureParser parser = new FeatureParser();
        FeatureFile featureFile = parser.parse("crlf.feature",
                new ByteArrayInputStream(content.getBytes(StandardCharsets.UTF_8)));
        ...
        assertThat(context.getIssues()).hasSize(1);
        assertThat(context.getIssues().get(0).message())
                .contains("This file has 3 lines");
    }
