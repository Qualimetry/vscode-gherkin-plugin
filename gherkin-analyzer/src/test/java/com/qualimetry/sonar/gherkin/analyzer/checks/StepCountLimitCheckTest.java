...
class StepCountLimitCheckTest {

    @Test
    void shouldNotRaiseIssueWithinLimit() {
        CheckVerifier.verifyNoIssues(
                new StepCountLimitCheck(),
                "checks/step-count-limit/compliant.feature");
    }
    ...
    void shouldRaiseIssueWhenExceedingLimit() {
        CheckVerifier.verify(
                new StepCountLimitCheck(),
                "checks/step-count-limit/noncompliant.feature");
    }

    @Test
    void shouldRaiseIssueWithCustomLimit() {
        StepCountLimitCheck check = new StepCountLimitCheck();
        check.setMaxSteps(3);
        CheckVerifier.verify(check,
                "checks/step-count-limit/custom-limit-noncompliant.feature");
    }
}
