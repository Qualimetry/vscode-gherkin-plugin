 ...
 * <p>
 * {@code And} / {@code But} continuations inherit the type of the last
 * non-conjunction step before them, so a Then expressed as a trailing
 * {@code And} counts as an outcome. Background steps are not counted;
 * Backgrounds are Given-only by convention.
 */
    ...
    public void leaveScenario(ScenarioDefinition scenario) {
        ...
        for (StepDefinition step : scenario.steps()) {
            if (!"CONJUNCTION".equals(step.keywordType())) {
                effectiveType = step.keywordType();
            }
            if ("ACTION".equals(effectiveType)) {
                hasWhen = true;
            } else if ("OUTCOME".equals(effectiveType)) {
                hasThen = true;
            }
        }
        if (!hasWhen && !hasThen) {
