...
/**
 * Checks that <em>Then</em> steps match a configurable regular-expression
 * pattern. And/But steps continuing a Then are checked as well.
 * <p>
 * This rule allows teams to enforce naming conventions on outcome steps,
 * such as requiring them to start with a specific phrase or follow a
 * particular sentence structure.
 */
@Rule(key = "then-step-pattern")
public class ThenStepPatternCheck extends BaseCheck {

    private static final String DEFAULT_PATTERN = ".*";

    @RuleProperty(
            key = "pattern",
            description = "Regular expression that Then step text must match",
    ...
    public void visitStep(StepDefinition step) {
        ...
        if (compiledPattern == null || !"OUTCOME".equals(effectiveKeywordType)) {
            return;
        }
        if (!compiledPattern.matcher(step.text()).matches()) {
            addIssue(step.position(),
                    "Then step does not match the required pattern: " + pattern);
        }
    }
}
