...
public class OutlinePlaceholderRequiredCheck extends BaseCheck {
    ...
    public void leaveScenario(ScenarioDefinition scenario) {
        ...
        boolean hasPlaceholder = containsPlaceholder(scenario.name())
                || containsPlaceholder(scenario.description());

        if (!hasPlaceholder) {
            for (StepDefinition step : scenario.steps()) {
                if (stepContainsPlaceholder(step)) {
                    hasPlaceholder = true;
                    break;
                }
            }
    ...
    private static boolean stepContainsPlaceholder(StepDefinition step) {
        if (containsPlaceholder(step.text())) {
            return true;
        }
        if (step.docString() != null && containsPlaceholder(step.docString().content())) {
            return true;
        }
        if (step.dataTable() != null) {
            for (var row : step.dataTable().rows()) {
                for (String cell : row) {
