...
/**
 * Checks that comments do not contain to-do markers.
 * <p>
 * Such comments indicate unfinished work that should be tracked in an
 * issue tracker rather than left as comments in feature files. Leaving
 * them in Gherkin files risks them being forgotten.
 */
@Rule(key = "todo-comment")
public class TodoCommentCheck extends BaseCheck {

    private static final Pattern TODO_PATTERN =
            Pattern.compile("\\bto[\\s_-]?do\\b", Pattern.CASE_INSENSITIVE);

    @Override
    public void visitComment(Comment comment) {
        if (TODO_PATTERN.matcher(comment.text()).find()) {
            addIssue(comment.position(),
                    "Complete or remove this TODO comment.");
        }
    }
}
