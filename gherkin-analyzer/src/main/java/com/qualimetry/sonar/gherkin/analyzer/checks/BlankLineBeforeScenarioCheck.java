...
public class BlankLineBeforeScenarioCheck extends BaseCheck {
    ...
    public void visitScenario(ScenarioDefinition scenario) {
        ...
        // Skip the tag/comment block directly above; the blank line must
        // precede the whole block.
        int prevIndex = effectiveLine - 2;
        while (prevIndex >= 0 && prevIndex < lines.length
                && isTagOrCommentLine(lines[prevIndex])) {
            prevIndex--;
        }
    ...
    private static boolean isTagOrCommentLine(String line) {
