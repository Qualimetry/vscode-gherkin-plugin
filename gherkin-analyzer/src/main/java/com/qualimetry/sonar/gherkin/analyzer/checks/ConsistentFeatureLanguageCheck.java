 ...
 * vary. The expected language is the one used by the file whose URI sorts
 * first lexicographically, so results do not depend on visit order.
 * <p>
...
public class ConsistentFeatureLanguageCheck extends BaseCheck {
     ...
     * cross-file issues for language inconsistencies. The baseline language
     * is taken from the lexicographically-first file URI among analyzed
     * files, making the result independent of visit order.
     *
    ...
    public List<CrossFileIssue> afterAllFiles() {
        ...
        String expectedLanguage = occurrences.stream()
                .min(Comparator.comparing(LanguageOccurrence::uri))
                .map(LanguageOccurrence::language)
                .orElseThrow();
