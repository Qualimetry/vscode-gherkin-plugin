 ...
 *   <li>Doc-string delimiters: step level + 1 (content lines are not checked)</li>
 * </ul>
 */
...
public class ConsistentIndentationCheck extends BaseCheck {
    ...
    public void visitStep(StepDefinition step) {
        ...
        checkDocStringDelimiters(step.docString(), level + 1);
    }

    private void checkDocStringDelimiters(DocStringDefinition docString, int expectedLevel) {
        if (docString == null) {
            return;
        }
        int openingLine = docString.position().line();
        checkIndentation(docString.position(), expectedLevel);

        String content = docString.content();
        int contentLineCount = content.isEmpty() ? 0 : content.split("\n", -1).length;
        int closingLine = openingLine + contentLineCount + 1;
        checkIndentation(new TextPosition(closingLine, 1), expectedLevel);
    }
