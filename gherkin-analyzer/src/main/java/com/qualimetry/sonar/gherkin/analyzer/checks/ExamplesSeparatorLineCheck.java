 ...
 * by visually separating the scenario steps from the data table. Tag lines
 * directly above the Examples keyword belong to the Examples section, so
 * the blank line is expected above them.
 */
...
public class ExamplesSeparatorLineCheck extends BaseCheck {
    ...
    public void visitExamples(ExamplesDefinition examples) {
        ...
        // skipping tag lines that belong to the Examples section.
        int prevIndex = examplesLine - 2;
        while (prevIndex >= 0 && prevIndex < lines.length && isTagOnlyLine(lines[prevIndex])) {
            prevIndex--;
        }
    ...
    private static boolean isTagOnlyLine(String line) {
