 ...
 * consecutive {@code @}-prefixed tokens must be separated by exactly one ASCII
 * space. Tabs, multiple spaces, mixed whitespace, or no separator at all make
 * the tag line harder to read.
 * <p>
...
public class OneSpaceBetweenTagsCheck extends BaseCheck {
    ...
    /**
     * Captures the whitespace separator between two tag tokens. A compliant
     * separator is exactly one ASCII space.
     */
    private static final Pattern TAG_SEPARATOR = Pattern.compile("@[^@\\s]+(\\s*)(?=@)");
    ...
    public void visitFeatureFile(FeatureFile file) {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (TAG_LINE.matcher(line).find()
                    && (hasNonSingleSpaceSeparator(line) || MISSING_SPACING.matcher(line).find())) {
                addLineIssue(i + 1, "Use exactly one space between tags on this line.");
            }
        }
    }

    private static boolean hasNonSingleSpaceSeparator(String line) {
        Matcher matcher = TAG_SEPARATOR.matcher(line);
        while (matcher.find()) {
