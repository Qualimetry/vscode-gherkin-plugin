...
- Connect a workspace to its SonarQube project and see the server's open issues alongside local findings.
- Mark a finding as false positive or accepted on the server from the editor.
- Read the full rule description from a finding or with the "Gherkin: Show rule description" command.
- Clearer messages when a SonarQube quality profile is missing, empty, or uses a different analyzer.
