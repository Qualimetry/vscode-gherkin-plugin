...
- `examples-column-coverage` - requires every column to be used by the outline.
- `no-empty-examples-cells` - requires every cell to carry a value.
- `outline-placeholder-required` - requires the outline to reference its columns.
