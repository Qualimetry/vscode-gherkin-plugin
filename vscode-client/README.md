...
Use the command **Gherkin: Connect to SonarQube** (Command Palette or right-click). Enter your SonarQube server URL and a token if the server requires authentication. The extension finds the SonarQube project that matches your workspace (from scanner configuration or the Git remote, or you pick it from a list), binds the workspace to it, and imports the rules and severities activated in that project's Gherkin quality profile into `gherkinAnalyzer.rules`.

Once bound, the extension also shows the project's open SonarQube issues alongside local findings. Findings already marked false positive or accepted on the server are hidden by default (`gherkinAnalyzer.sonar.hideResolvedIssues`), and a finding can be marked false positive or accepted on the server from its code actions. Run **Gherkin: Sync issues from SonarQube** to refresh, or set `gherkinAnalyzer.sonar.issueSync` to `false` to turn issue sync off. Use `gherkinAnalyzer.sonar.branch` to read issues from a branch other than the checked-out one, and `gherkinAnalyzer.sonar.pathPrefix` when the workspace is opened at a subdirectory of the scanned project.

**Privacy and storage:** The server URL is saved as soon as you enter it (so it appears pre-filled next time) in the extension's global state, not in your settings files. The token (if provided) is stored securely in VS Code's secret storage. If the token input closes when you switch to another app to copy the token, run the command again - the URL will already be filled in and you only need to paste the token.

**Automatic re-sync:** After a first successful connection, the extension re-imports rules from the same server each time it starts, so profile changes in SonarQube propagate without a manual re-import. Set `gherkinAnalyzer.sonar.autoSyncOnStartup` to `false` to disable this.

### Rule descriptions

Choose **Explain rule** from a finding's code actions, or run **Gherkin: Show rule description**, to read why the rule exists, how to fix the finding, and related rules.
...
- **[SonarQube plugin](https://github.com/Qualimetry/sonarqube-gherkin-plugin)** - same rules in CI/CD quality gates.
- **[IntelliJ plugin](https://github.com/Qualimetry/intellij-gherkin-plugin)** - same rules in JetBrains IDEs and Qodana.
