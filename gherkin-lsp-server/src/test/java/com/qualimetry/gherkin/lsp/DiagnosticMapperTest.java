...
class DiagnosticMapperTest {
    ...
    void issueWithPositionMapsTo0Based() {
        Issue issue = new Issue("parse-error", "Unexpected token", new TextPosition(5, 10), 5, null, null);
    ...
    void issueWithOnlyLineMapsTo0Based() {
        Issue issue = new Issue("scenario-required", "Missing scenario", null, 3, null, null);
    ...
    void issueWithNeitherPositionNorLineDefaultsToZero() {
        Issue issue = new Issue("feature-file-required", "No feature keyword", null, null, null, null);
    ...
    void severityIsSetFromSeverityMap() {
        Issue errorIssue = new Issue("parse-error", "msg", null, 1, null, null);
        Issue warningIssue = new Issue("scenario-required", "msg", null, 1, null, null);
        Issue infoIssue = new Issue("consistent-indentation", "msg", null, 1, null, null);
        Issue hintIssue = new Issue("todo-comment", "msg", null, 1, null, null);
    ...
    @Test
    void sourceIsQualimetryGherkin() {
        Issue issue = new Issue("parse-error", "msg", null, 1, null, null);

        Diagnostic diag = DiagnosticMapper.toDiagnostic(issue);

        assertThat(diag.getSource()).isEqualTo("qualimetry-gherkin");
    }

    @Test
    void codeIsRuleKey() {
        Issue issue = new Issue("step-required", "msg", null, 1, null, null);
    ...
    @Test
    void codeDescriptionPointsToRuleDocumentation() {
        Issue issue = new Issue("step-required", "msg", null, 1, null, null);

        Diagnostic diag = DiagnosticMapper.toDiagnostic(issue);

        assertThat(diag.getCodeDescription()).isNotNull();
        assertThat(diag.getCodeDescription().getHref())
                .isEqualTo("https://github.com/Qualimetry/sonarqube-gherkin-plugin/blob/main/docs/rules/step-required.md");
    }

    @Test
    void crossFileIssueCodeDescriptionPointsToRuleDocumentation() {
        CrossFileIssue issue = new CrossFileIssue(
                "unique-feature-name", "file:///features/login.feature", 7, "Duplicate feature name");
        ...
        assertThat(diag.getCodeDescription()).isNotNull();
        assertThat(diag.getCodeDescription().getHref())
                .isEqualTo("https://github.com/Qualimetry/sonarqube-gherkin-plugin/blob/main/docs/rules/unique-feature-name.md");
    }

    @Test
    void issueWithEndColumn_highlightsWordRange() {
        Issue issue = new Issue("business-language-only", "Replace the technical term \"click\" with business-level language.",
                new TextPosition(7, 11), 7, null, 15);

        Diagnostic diag = DiagnosticMapper.toDiagnostic(issue);

        assertThat(diag.getRange().getStart().getLine()).isEqualTo(6);
        assertThat(diag.getRange().getStart().getCharacter()).isEqualTo(10);
        assertThat(diag.getRange().getEnd().getLine()).isEqualTo(6);
        assertThat(diag.getRange().getEnd().getCharacter()).isEqualTo(15);
    }
}
