...
public final class DiagnosticMapper {
    ...
    private static final String SOURCE = "qualimetry-gherkin";
    ...
    /**
     * Converts an analyzer {@link Issue} to an LSP {@link Diagnostic} using
     * the given rule configuration for severity (when non-null).
     *
     * @param issue the analyzer issue
     * @param ruleConfig optional rule configuration for severity override
     * @return an LSP diagnostic
     */
    public static Diagnostic toDiagnostic(Issue issue, RuleConfiguration ruleConfig) {
        int startLine;
        int startCol;
        ...
        Range range = endColumn != null
                ? buildRange(startLine, startCol, endColumn)
                : buildRange(startLine, startCol);

        Diagnostic diagnostic = new Diagnostic();
        diagnostic.setRange(range);
        diagnostic.setMessage(issue.message());
        diagnostic.setSeverity(ruleConfig != null ? ruleConfig.getSeverity(issue.ruleKey()) : SeverityMap.getSeverity(issue.ruleKey()));
        diagnostic.setSource(SOURCE);
        diagnostic.setCode(issue.ruleKey());
        diagnostic.setCodeDescription(new DiagnosticCodeDescription(RuleHelpUrls.helpUrl(issue.ruleKey())));
        return diagnostic;
    }
    ...
    /**
     * Converts a {@link CrossFileIssue} to an LSP {@link Diagnostic} using
     * the given rule configuration for severity (when non-null).
     *
     * @param issue the cross-file issue
     * @param ruleConfig optional rule configuration for severity override
     * @return an LSP diagnostic
     */
    public static Diagnostic toDiagnostic(CrossFileIssue issue, RuleConfiguration ruleConfig) {
    ...
    private static Range buildRange(int startLine, int startCol, int endColumn1Based) {
        Position start = new Position(startLine, startCol);
        Position end = new Position(startLine, endColumn1Based);
        return new Range(start, end);
    }
}
